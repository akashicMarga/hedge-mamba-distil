...
def main():
    ...
    parser.add_argument("--grad_accumulation",type=int,  default=4,
                        help="Accumulate N micro-batches before optimizer step")
    ...
    if args.debug:
        ...
        args.state_size        = args.state_size or 64  # prevent OOM with Ns=2048
        args.log_every         = 2
        args.eval_every        = 10
        print("[run_parler_stage1] --debug: 1 epoch, batch=1, grad_accum=2, max_audio=128, state_size=64", flush=True)
    ...
    from src.mlx.parler_model import ParlerMambaMLX, apply_weight_surgery_mlx
    ...
    # Warm-start SSM from teacher attention weights (paper Appendix B)
    print("[run_parler_stage1] Applying weight surgery ...", flush=True)
    apply_weight_surgery_mlx(student, teacher)
